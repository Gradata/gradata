...
def run_one_session(
    session_id: int, scenario: Scenario, brain_dir: Path, distractor_count: int = 5
) -> SessionResult:
    t0 = time.time()
    try:
        ...
        recall_at_1 = bool(rules_text) and any(
            kw.lower() in text_lower for kw in scenario.expected_keywords
        )
...
def run_benchmark(
    num_sessions: int, distractor_count: int, seed: int, parallel: int = 1
) -> BenchResult:
    random.seed(seed)
    ...
    print(
        f"[pmr-100] running {num_sessions} sessions, {distractor_count} distractors each, seed={seed}",
        file=sys.stderr,
    )
    ...
    # Save JSON
    out_path = out_dir / f"pmr_100_{datetime.now(UTC).strftime('%Y%m%dT%H%M%S')}.json"
    out_path.write_text(
        json.dumps(
            {
...
def print_one_paragraph(bench: BenchResult) -> None:
    s = bench.summary
    print()
    print("PMR-100 Procedural Memory Retention benchmark — Gradata")
...
def main() -> None:
    ...
    p.add_argument(
        "--quick", action="store_true", help="10 sessions for fast feedback (default: 100)"
    )
    p.add_argument(
        "-n",
        "--num-sessions",
    ...
    p.add_argument(
        "--distractors", type=int, default=5, help="Distractor turns per session (default: 5)"
    )
