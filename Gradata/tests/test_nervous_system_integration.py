...
class TestNervousSystemIntegration:
    ...
    def test_rule_ranker_with_context(self, tmp_path):
        ...
        rules = [
            {"description": "validate email", "confidence": 0.8, "category": "SALES", "fire_count": 3, "last_session": 5},
            {"description": "clamp confidence", "confidence": 0.8, "category": "CODE", "fire_count": 3, "last_session": 5},
        ]
        ranked = rank_rules(rules, current_session=10, context_keywords=["confidence", "graduation"])
    ...
    def test_session_history_wires_to_bus(self, tmp_path):
        brain = init_brain(tmp_path)
        brain.bus.emit("rules.injected", {"rules": [
