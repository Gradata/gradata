...
class TestSeverityWeightsExist:
    ...
    def test_rewrite_penalty_math(self):
        """Rewrite: base * 1.30 should be the harshest severity."""
        effective = abs(CONTRADICTION_PENALTY) * SEVERITY_WEIGHTS["rewrite"]
        # Calibrated: 0.10 * 1.30 = 0.13 (rewrite is always harshest)
        assert effective > abs(CONTRADICTION_PENALTY) * SEVERITY_WEIGHTS["major"]
...
class TestContradictionSeverity:
    ...
    def test_rewrite_correction_large_penalty(self):
        ...
        drop = original - lesson.confidence
        # Calibrated: 0.10 * 1.30 * (0.5 + 0.50*0.5) = 0.0975 (FSRS scaling)
        assert drop > 0.08, f"Rewrite penalty too small: {drop}"
