...
class TestEventsModule:
    ...
    def test_emit_gate_result(self, tmp_path):
        ...
        event = emit_gate_result(
            "demo-prep", "PASS", sources_checked=["LinkedIn", "web"], detail="OK"
        )
    ...
    def test_compute_leading_indicators_calibration_v1(self, tmp_path):
        ...
        brain = _init_brain(tmp_path)
        brain.emit(
            "CALIBRATION", "pytest", {"delta": 0, "self_score": 8, "reviewer_score": 8}, session=5
        )
        brain.emit(
            "CALIBRATION", "pytest", {"delta": 1, "self_score": 7, "reviewer_score": 8}, session=5
        )
    ...
    def test_compute_leading_indicators_calibration_v2(self, tmp_path):
        ...
        brain.emit(
            "CALIBRATION",
            "pytest",
            {"brier_score": 0.04, "calibration_rating": "EXCELLENT"},
            session=6,
        )
...
class TestQueryModule:
        ...
        docs = [
            {
                ...
                "embed_date": "2026-01-01",
            },
            {
                "source": "sessions/s2.md",
                "file_type": "session",
                "text": "Follow-up action items.",
                "embed_date": "2026-01-02",
            },
        ]
    ...
    def test_fts_search_file_type_filter(self, tmp_path):
        ...
        docs = [
            {
                "source": "prospects/A.md",
                "file_type": "prospect",
                "text": "AI tooling budget.",
                "embed_date": "2026-01-01",
            },
            {
                "source": "sessions/S1.md",
                "file_type": "session",
                "text": "AI tooling notes.",
                "embed_date": "2026-01-01",
            },
        ]
        fts_index_batch(docs)
    ...
    def test_compute_recency_weight_old(self):
        from gradata._query import RECENCY_FLOOR, compute_recency_weight
    ...
    def test_infer_memory_type_fallback_to_map(self):
        from gradata._query import MEMORY_TYPE_MAP, infer_memory_type
    ...
    def test_brain_search_keyword_mode(self, tmp_path):
        ...
        from gradata._query import brain_search, fts_rebuild
...
class TestExportBrainModule:
    ...
    def test_build_prospect_map_with_files(self, tmp_path):
        ...
        (prospects / "Bob Smith.md").write_text("name: Bob Smith\n", encoding="utf-8")
...
class TestOnboard:
    ...
    def test_onboard_creates_company_md(self, tmp_path):
        _init_brain(tmp_path, name="CompanyBrain")
    ...
    def test_onboard_invalid_embedding_falls_back(self, tmp_path):
        ...
        brain = onboard(
            brain_dir, name="FallbackBrain", domain="General", embedding="openai", interactive=False
        )
    ...
    def test_onboard_gemini_without_key_proceeds(self, tmp_path, capsys):
        ...
        brain = onboard(
            brain_dir, name="GeminiBrain", domain="General", embedding="gemini", interactive=False
        )
