...
def test_extraction_skipped_when_converged(tmp_path):
    ...
    converged_result = {
        ...
        "by_category": {
            "DRAFTING": {
                "corrections_per_session": [5, 4, 2, 2, 2],
                "trend": "converged",
                "p_value": 0.9,
            },
        },
...
def test_extraction_runs_when_diverging(tmp_path):
    ...
    with (
        patch.object(brain, "_get_convergence", return_value=diverging_result),
        patch(
            "gradata.enhancements.behavioral_extractor.extract_instruction",
            return_value="Use 'well' instead of 'good'",
        ) as mock_extract,
    ):
        brain.correct(
            "The system is working good",
...
def test_convergence_cache_per_session(tmp_path):
    ...
    with patch(
        "gradata._core.brain_convergence", return_value={"trend": "converging", "by_category": {}}
    ) as mock_conv:
