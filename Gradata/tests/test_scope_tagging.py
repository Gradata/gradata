...
def test_scope_in_lesson_scope_json(tmp_path: Path):
    ...
    assert len(new_lessons) >= 1, (
        f"No lesson with correction_scope found. scope_jsons: {[l.scope_json for l in lessons]}"
    )
...
def test_one_off_never_graduates_past_instinct():
    ...
    active, _graduated = graduate([lesson])
...
def test_applies_to_propagates_to_lesson_scope_json(tmp_path: Path):
    ...
    text = lessons_path.read_text(encoding="utf-8")
    lessons = parse_lessons(text)
    tagged = [l for l in lessons if l.scope_json and "applies_to" in l.scope_json]
