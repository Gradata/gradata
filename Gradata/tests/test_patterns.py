...
class TestOrchestrator:
    ...
    def test_classify_request_returns_classification_object(self):
        from gradata.contrib.patterns.orchestrator import RequestClassification, classify_request
    ...
    def test_classify_request_has_selected_pattern(self):
        from gradata.contrib.patterns.orchestrator import ALL_PATTERNS, classify_request
...
class TestPipeline:
    ...
    def test_stage_runs_handler(self):
        ...
        output, _gate_result, _retries = stage.run(5)
        assert output == 10
    ...
    def test_gate_decorator_wraps_bool_function(self):
        from gradata.contrib.patterns.pipeline import GateResult, gate
...
class TestReflection:
    ...
    def test_critique_checklist_duplicate_names_raises(self):
        from gradata.contrib.patterns.reflection import Criterion, CritiqueChecklist
    ...
    def test_reflect_max_cycles_less_than_one_raises(self):
        from gradata.contrib.patterns.reflection import EMAIL_CHECKLIST, default_evaluator, reflect
    ...
    def test_default_evaluator_scores_non_empty_string(self):
        from gradata.contrib.patterns.reflection import Criterion, CriterionScore, default_evaluator
...
class TestGuardrails:
    ...
    def test_pii_detector_catches_email(self):
        from gradata.contrib.patterns.guardrails import GuardCheck, pii_detector
    ...
    def test_guarded_blocks_pii_input(self):
        from gradata.contrib.patterns.guardrails import InputGuard, guarded, pii_detector
    ...
    def test_guarded_result_has_all_passed_field(self):
        from gradata.contrib.patterns.guardrails import GuardedResult, guarded
...
class TestManifestGuardrails:
    ...
    def test_validate_spawn_within_budget(self):
        ...
        result = validate_agent_spawn(20000, budget_enabled=True, parent_budget_remaining=100000)
        assert result.allowed
    ...
    def test_validate_spawn_denied_hard_limit(self):
        ...
        result = validate_agent_spawn(
            95000,
            budget_enabled=True,
            parent_budget_remaining=100000,
    ...
    def test_validate_spawn_warning(self):
        ...
        result = validate_agent_spawn(
            ...
            budget_enabled=True,
            parent_budget_remaining=100000,
            child_warning_threshold_percent=80,
            child_hard_limit_percent=95,
        )
        assert result.allowed
...
class TestMemoryScope:
    ...
    def test_path_takes_priority_over_event_type(self):
        ...
        assert (
            classify_memory_scope(source_path="metrics/daily.json", event_type="OUTPUT") == "user"
        )
...
class TestMemory:
    ...
    def test_semantic_memory_conflict_resolve_picks_newer(self):
        from gradata.contrib.patterns.memory import Memory, SemanticMemory
...
class TestEvaluator:
    ...
    def test_evaluate_empty_dimensions_raises(self):
        from gradata.contrib.patterns.evaluator import default_evaluator, evaluate
    ...
    def test_evaluate_verdicts_are_known_values(self):
        ...
        result = evaluate(
            "Some output text.", dimensions=QUALITY_DIMENSIONS, evaluator=default_evaluator
        )
...
class TestParallel:
    ...
    def test_merge_results_combine_strategy(self):
        from gradata.contrib.patterns.parallel import TaskResult, merge_results
...
class TestHumanLoop:
    ...
    def test_assess_risk_returns_risk_assessment(self):
        from gradata.contrib.patterns.human_loop import RiskAssessment, assess_risk
    ...
    def test_gate_low_risk_auto_approved_returns_none(self):
        from gradata.contrib.patterns.human_loop import assess_risk, gate
    ...
    def test_gate_high_risk_returns_approval_request(self):
        from gradata.contrib.patterns.human_loop import ApprovalRequest, assess_risk, gate
...
class TestSubAgents:
    ...
    def test_orchestrate_dispatches_to_handlers(self):
        from gradata.contrib.patterns.sub_agents import Delegation, OrchestratedResult, orchestrate
    ...
    def test_orchestrate_unknown_agent_records_failure(self):
        from gradata.contrib.patterns.sub_agents import Delegation, orchestrate
    ...
    def test_orchestrate_empty_delegations(self):
        from gradata.contrib.patterns.sub_agents import OrchestratedResult, orchestrate
...
class TestRag:
    ...
    def test_apply_graduation_scoring_boosts_rules(self):
        from gradata.contrib.patterns.rag import Chunk, apply_graduation_scoring
        ...
        chunks = [
            Chunk(
                ...
                source="patterns.md",
                relevance_score=0.5,
                graduation_level="RULE",
            ),
            Chunk(
                content="INSTINCT content",
                source="lessons.md",
                relevance_score=0.5,
                graduation_level="INSTINCT",
            ),
        ]
    ...
    def test_order_by_relevance_position_sorts_highest_first(self):
        ...
        [c.relevance_score for c in ordered]
    ...
    def test_cascade_retrieve_returns_retrieval_result(self):
        from gradata.contrib.patterns.rag import Chunk, RetrievalResult, cascade_retrieve

        def mock_fts(query, limit):
            return [
                Chunk(content=f"fts:{query}", source="fts", relevance_score=0.9, chunk_id="fts1")
            ]
...
class TestScope:
    ...
    def test_classify_scope_detects_research_intent(self):
        ...
        task_name, _audience = classify_scope("research this company and find info")
        assert task_name == "research"

    def test_classify_scope_empty_input_returns_general(self):
        from gradata.rules.scope import classify_scope

        task_name, _audience = classify_scope("")
        assert task_name == "general"

    def test_classify_scope_detects_audience_vp(self):
        from gradata.rules.scope import AudienceTier, classify_scope

        _task_name, audience = classify_scope("VP of Sales at a 500-person enterprise company")
...
class TestMCP:
    ...
    def test_create_brain_mcp_tools_have_names(self):
        from gradata.contrib.patterns.mcp import MCPToolSchema, create_brain_mcp_tools
