...
class GraduationEngine:
    """Protocol for pluggable graduation engines.

    Implement this interface to replace the default graduation logic.
    Pass your implementation to Brain(graduation_engine=MyEngine()).

    Methods:
        update_confidence: Adjust lesson confidence after a correction round.
        graduate: Apply state transitions (promote/demote/kill) to lessons.
    """
    ...
    def update_confidence(
        ...
        *,
        severity_data: dict | None = None,
        is_machine: bool = False,
    ) -> list[Lesson]:
        """Adjust confidence on lessons based on new corrections.
    ...
    def graduate(
        ...
        *,
        ...
        """Apply state transitions. Returns (active, graduated).

        Default implementation: gradata.enhancements.self_improvement.graduate
        """
        from gradata.enhancements.self_improvement import graduate
        return graduate(lessons, maturity=maturity, machine_mode=machine_mode)
