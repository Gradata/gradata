...
def compute_brain_scores(last_n_prospect_sessions: int = 10) -> dict:
    ...
    with contextlib.closing(sqlite3.connect(str(_p.DB_PATH))) as conn:
        ...
        # --- Compound Growth (domain-specific business outcomes) ---
        # Domain plugins can populate custom metrics tables (e.g. daily_metrics,
        # pipeline_snapshots, deals). The SDK does not assume any specific schema.
        # Compound growth falls back to pattern accumulation when no domain
        # tables are present.
