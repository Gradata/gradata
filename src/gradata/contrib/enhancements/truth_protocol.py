"""Evidence-based output validation (stdlib). ``verify_*``→``TruthVerdict``; banned-phrase regex;
``numbers_without_source`` flags bare percentages; ``verify_mutations`` requires ``|`` or ``->``."""
