"""
Onboarding wizard for bootstrapping a new Gradata.
...
# ── Subdirectories ────────────────────────────────────────────────────
# Core dirs (always created) + domain-specific dirs (configurable).

_CORE_SUBDIRS = ["sessions", "scripts"]

_DOMAIN_SUBDIRS = {
    "sales": ["prospects", "emails", "vault"],
    "recruiting": ["candidates", "roles", "vault"],
    "engineering": ["projects", "reviews", "vault"],
    "support": ["customers", "tickets", "vault"],
}

# Default includes sales dirs for backward compat
...
def _build_manifest(name: str, domain: str, embedding: str) -> dict:
    ...
    return {
        ...
        "bootstrap": [
            ...
            {
                "step": "init_db",
                "command": "gradata init .",
                "required": True,
            },
            {
                "step": "embed_brain",
                "command": "gradata embed --full",
                "required": True,
            },
        ],
        "compatibility": {
...
def onboard(
    ...
    *,
    ...
    # ── Workflow discovery (interactive only) ──────────────────────────
    # Ask about common workflows to create seed rules so the brain
    # isn't empty on day 1. Inspired by Hermes Agent's setup wizard.
    ...
    # Subdirectories — domain-aware
    domain_lower = (domain or "sales").lower()
    subdirs = _CORE_SUBDIRS + _DOMAIN_SUBDIRS.get(domain_lower, _DOMAIN_SUBDIRS["sales"])
    for sub in subdirs:
    ...
    # DOMAIN.md — the brain's identity and rules
    domain_md = brain_dir / "DOMAIN.md"
