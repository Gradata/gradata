...
def check_correction_safety(
    ...
    """Pre-graduation safety check against existing lessons.

    Reads the lessons file, extracts graduated rules (PATTERN + RULE),
    and checks the new correction for contradictions.

    Args:
        correction_text: The new correction/lesson text to check.
        lessons_path: Path to lessons.md.

    Returns:
        Dict with:
        - safe: bool — True if no high-confidence contradictions
        - contradictions: list of contradiction dicts
        - recommendation: str — human-readable recommendation
    """
    if not lessons_path.exists():
        return {"safe": True, "contradictions": [], "recommendation": "No lessons file found; safe to proceed."}

    text = lessons_path.read_text(encoding="utf-8")
    existing_rules = _parse_lessons_to_rules(text)

    contradictions = detect_contradictions(correction_text, existing_rules)

    # Filter to high-confidence contradictions only
    high_conf = [c for c in contradictions if c["confidence"] > 0.7]

    if not high_conf:
        return {
            "safe": True,
            "contradictions": contradictions,
            "recommendation": "No high-confidence contradictions found. Safe to graduate.",
        }

    # Build recommendation
    conflict_cats = [c["existing_rule"].get("category", "?") for c in high_conf]
    recommendation = (
        f"HOLD: {len(high_conf)} contradiction(s) detected with categories: "
        f"{', '.join(conflict_cats)}. Review before graduating. "
        f"The new rule may need to supersede or be reconciled with existing rules."
    )

    return {
        "safe": False,
        "contradictions": high_conf,
        "recommendation": recommendation,
    }


def _parse_lessons_to_rules(text: str) -> list[dict]:
