...
def detect_conflicts(lessons: list[Lesson]) -> list[tuple[Lesson, Lesson, float]]:
    """Find pairs of graduated lessons that may contradict each other.

    Uses semantic similarity + antonym detection. Two lessons conflict if:
    - Same category (both TONE corrections)
    - High semantic similarity (same topic)
    - Opposite direction (one says "formalize", other says "casualize")

    Returns:
        List of (lesson_a, lesson_b, conflict_score) tuples, sorted by
        conflict_score descending. Score > 0.5 = likely conflict.
    """
    try:
        from gradata.enhancements.similarity import semantic_similarity
    except ImportError:
    ...
    # Direction-opposite patterns: if both appear in same-category lessons,
    # they likely contradict
    _OPPOSING_PAIRS = [
        ({"formal", "formalized", "professional", "polished"},
         {"casual", "casualized", "informal", "relaxed", "conversational"}),
        ({"strengthen", "strengthened", "direct", "blunt", "assertive"},
         {"soften", "softened", "gentle", "diplomatic", "tactful"}),
        ({"short", "shorter", "brief", "concise", "terse", "cut"},
         {"longer", "detailed", "elaborate", "expand", "more"}),
        ({"formal", "serious", "corporate"},
         {"friendly", "warm", "personal", "human"}),
    ]

    eligible = [l for l in lessons if l.state in ELIGIBLE_STATES]
    conflicts: list[tuple[Lesson, Lesson, float]] = []
    ...
    for i, a in enumerate(eligible):
        for b in eligible[i + 1:]:
            ...
            sim = semantic_similarity(a.description, b.description)
            if sim < 0.2:
                continue  # Too different to conflict
            ...
            for pos_set, neg_set in _OPPOSING_PAIRS:
                a_has_pos = any(w in a_lower for w in pos_set)
                a_has_neg = any(w in a_lower for w in neg_set)
                b_has_pos = any(w in b_lower for w in pos_set)
                b_has_neg = any(w in b_lower for w in neg_set)

                # Conflict: A says positive, B says negative (or vice versa)
                if (a_has_pos and b_has_neg) or (a_has_neg and b_has_pos):
                    opposition_score = max(opposition_score, 0.8)
                    break

            if opposition_score > 0:
                conflict_score = (sim + opposition_score) / 2
                conflicts.append((a, b, conflict_score))
