...
def format_rules_for_prompt(
    ...
    for rule in rules:
        ...
        draft = getattr(lesson, "example_draft", None)
        corrected = getattr(lesson, "example_corrected", None)
        if needs_reinforcement and draft is not None and corrected is not None:
            lines.append(
                f'   e.g. "{draft[:80]}" -> "{corrected[:80]}"'
            )
