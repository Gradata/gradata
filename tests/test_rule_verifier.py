...
class TestAutoDetectVerification:
    ...
    def test_booking_link_rule(self):
...
class TestVerifyRules:
    ...
    def test_missing_booking_link_violation(self):
        rules = [{"category": "DRAFTING", "description": "Always include a booking link hyperlink"}]
        output = "Let me know if you want to chat."
        results = verify_rules(output, rules)
        violations = [r for r in results if not r.passed]
        assert len(violations) >= 1
        assert "booking" in violations[0].violation_detail

    def test_booking_link_present_passes(self):
        rules = [{"category": "DRAFTING", "description": "Always include a booking link"}]
