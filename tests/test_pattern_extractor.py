...
def _make_classification(category: str, description: str, confidence: float = 0.8,
                         severity: str = "minor"):
    ...
    try:
        ...
        from gradata.enhancements.edit_classifier import EditClassification  # type: ignore[assignment]
...
class TestPatternsToLessons:
    ...
    def test_lesson_initial_confidence_is_constant(self):
        from gradata.enhancements.pattern_extractor import patterns_to_lessons, INITIAL_CONFIDENCE
        p = self._make_pattern("TONE", "warmer language", confidence=0.40)
        result = patterns_to_lessons([p])
