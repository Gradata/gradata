"""
Unified event system for AIOS brain (SDK Copy).
=================================================
...
from aios_brain._stats import brier_score, rolling_comparison, wilson_ci
...
def emit(event_type: str, source: str, data: dict = None, tags: list = None,
    ...
    try:
        from aios_brain._tag_taxonomy import enrich_tags, validate_tags
    ...
    try:
        with open(_p.EVENTS_JSONL, "a", encoding="utf-8") as f:
            f.write(json.dumps(event, ensure_ascii=False) + "\n")
    except Exception as e:
        print(f"[events] JSONL write failed: {e}")
    ...
    try:
        ...
        print(f"[events] SQLite write failed: {e}")
...
def query(event_type: str = None, session: int = None, last_n_sessions: int = None,
          limit: int = 100, as_of: str = None, active_only: bool = False) -> list:
    conn = sqlite3.connect(str(_p.DB_PATH))
...
def compute_leading_indicators(session: int) -> dict:
    ...
    if outputs:
        total = len(outputs)
        unedited = sum(1 for r in outputs if not json.loads(r[0]).get("edited_by_oliver", False))
    ...
    if calibrations:
        ...
        #   v1 (sessions <= 6): {"delta": -1, "self_score": 7, "oliver_score": 6}
...
def _detect_session() -> int:
    import re
    brain_dir = _p.BRAIN_DIR
...
def compute_brain_scores(last_n_prospect_sessions: int = 10) -> dict:
    ...
    try:
        ...
        for row in scored_outputs:
            ...
            if self_score is not None:
                ...
                outcome = 0 if d.get("edited_by_oliver", False) else 1
    ...
    # Sources: daily_metrics (reply rate), deals table (pipeline build + velocity), pipeline_snapshots
    # Measures real CRM progression, not snapshot-to-snapshot deltas
    has_growth_data = False
    try:
        # 1. Reply rate from daily_metrics (Instantly data)
        ...
        if reply_row and reply_row[0]:
            reply_index = (reply_row[0] / 1.5) * 100  # 1.5% = 100% baseline

        # 2. Pipeline build rate from pipeline_snapshots (total value / weeks active)
        # Oliver started Feb 4, 2026 — measure pipeline accumulated per week
        pipeline_row = conn.execute(
            "SELECT SUM(value) FROM pipeline_snapshots WHERE owner = 'Oliver' "
            "AND stage NOT LIKE '%closed%' AND stage NOT LIKE '%lost%'"
        ).fetchone()
        pipeline_index = 100.0
        if pipeline_row and pipeline_row[0] and pipeline_row[0] > 0:
            total_pipeline = pipeline_row[0]
            # Weeks since start (Feb 4, 2026)
            weeks_active = max(1, (datetime.now() - datetime(2026, 2, 4)).days / 7)
            pipeline_per_week = total_pipeline / weeks_active
            # $500/week = baseline (100%). Scale linearly.
            pipeline_index = (pipeline_per_week / 500) * 100

        # 3. Deal velocity from deals table (avg days_in_stage for active deals)
        ...
        if velocity_row and velocity_row[0]:
            ...
            # 14 days = fast (150%), 30 days = baseline (100%), 60 days = slow (50%)
            # Cap at 200% — faster than 15 days is great but shouldn't dominate the score
