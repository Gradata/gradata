"""
Context Packet Builder (SDK Copy).
====================================
...
def _events_query(**kwargs):
    from aios_brain._events import query as eq
    return eq(**kwargs)
...
def _detect_session():
    from aios_brain._events import _detect_session as ds
    return ds()
        ...
        result["outputs"] = [
            {"ts": e.get("ts", "")[:19],
             "data": {k: v for k, v in e.get("data", {}).items()
                      if k in ("type", "self_score", "edited_by_oliver", "detail")}}
            for e in outputs
        ]
    except Exception:
        pass
