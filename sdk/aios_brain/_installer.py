...
Usage (via CLI):
    aios-brain install path/to/brain-archive.zip              # Install to default location
    aios-brain install path/to/brain-archive.zip --target DIR # Install to specific directory
    aios-brain install path/to/brain-archive.zip --dry-run    # Preview without installing
...
def _check_compatibility(manifest: dict) -> list[str]:
    ...
    # ChromaDB
    req_chroma = compat.get("chromadb")
    if req_chroma:
        try:
            import chromadb  # noqa: F401
        except ImportError:
            issues.append(f"chromadb {req_chroma} required — pip install chromadb")
...
def install(archive_path: Path, target_dir: Path = None, dry_run: bool = False) -> dict:
    ...
    if target_dir.exists() and any(target_dir.iterdir()):
        print(f"  WARNING: Target directory exists and is not empty")
    ...
    print(f"  # Or set in your runtime config")
