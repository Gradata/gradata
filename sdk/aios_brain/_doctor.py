"""
AIOS Brain Doctor — Environment and brain health diagnostics.
...
def _check_chromadb():
    ...
    try:
        ...
        return {"name": "chromadb", "status": "ok", "detail": version}
    except ImportError:
        return {
            "name": "chromadb",
            "status": "missing",
            "detail": "pip install chromadb",
        }
...
def _resolve_brain_path():
    ...
    try:
        from aios_brain._paths import resolve_brain_dir, DB_PATH
...
def print_diagnosis(report: dict) -> None:
    ...
    print(f"\n  AIOS Brain Doctor — {labels.get(status, status.upper())}")
    print(f"  {'=' * 46}")
