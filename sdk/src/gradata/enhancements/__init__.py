...
OPEN SOURCE modules (always available):
  diff_engine         -- Edit distance + 5-level severity
  quality_gates       -- 8.0 threshold with fix cycling
  truth_protocol      -- Evidence-based output validation

PROPRIETARY modules (also available via gradata_cloud when installed):
  self_improvement    -- INSTINCT->PATTERN->RULE graduation (user corrections)
  agent_graduation    -- Same graduation pipeline applied to agent/subagent outputs
  carl                -- Behavioral contracts per domain
  correction_tracking -- Density, half-life, MTBF
  brain_scores        -- Compound health metric (report card)
  ...
  loop_intelligence   -- Activity tracking, pattern aggregation, prep-to-outcome analysis
