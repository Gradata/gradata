...
Stolen from SuperMemory's 3-tool MCP pattern (memory, recall, whoAmI).
...
def correct(
    ...
    *,
    ...
    This is the core learning primitive. Every time a user edits an AI output,
...
def _load_lessons(lessons_path: str | Path | None = None) -> list[Lesson]:
    ...
    try:
        ...
        result = parse_lessons(path.read_text(encoding="utf-8"))
        if result:  # Stub returns [] -- fall back to regex parser
            return result
        return _parse_lessons_fallback(path)
    except (ImportError, Exception):
        # Fallback: basic regex parsing
        return _parse_lessons_fallback(path)


def _parse_lessons_fallback(path: Path) -> list[Lesson]:
    """Fallback lesson parser using regex."""
    text = path.read_text(encoding="utf-8")
    pattern = re.compile(
        r"\[([\d-]+)\]\s+\[(INSTINCT|PATTERN|RULE):([\d.]+)\]\s+(\w+):\s+(.+)"
    )
    lessons = []
    for m in pattern.finditer(text):
        date, state_str, conf_str, category, description = m.groups()
        try:
...
def _load_meta_rules(meta_rules_path: str | Path | None = None) -> list[dict]:
    ...
    if meta_rules_path is not None:
        ...
        try:
            import gradata._paths as _p
            path = _p.WORKING_DIR / ".claude" / "meta-rules.json"
        except Exception:
            return []
