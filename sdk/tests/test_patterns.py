...
class TestOrchestrator:
    ...
    def test_classify_request_returns_classification_object(self):
        from gradata.patterns.orchestrator import classify_request, RequestClassification
    ...
    def test_classify_request_has_selected_pattern(self):
        from gradata.patterns.orchestrator import classify_request, ALL_PATTERNS
        result = classify_request("draft an email to a prospect")
    ...
    def test_register_intent_pattern_replaces_existing(self):
        from gradata.patterns.orchestrator import (
    ...
    def test_register_intent_pattern_bad_pattern_raises(self):
        from gradata.patterns.orchestrator import register_intent_pattern
        with pytest.raises(ValueError):
            register_intent_pattern(intent="x", pattern="not_a_real_pattern")

    def test_all_patterns_constant_has_15_entries(self):
        from gradata.patterns.orchestrator import ALL_PATTERNS
...
class TestPipeline:
    ...
    def test_stage_runs_handler(self):
        from gradata.patterns.pipeline import Stage
    ...
    def test_gate_result_pass(self):
        from gradata.patterns.pipeline import GateResult
    ...
    def test_gate_decorator_wraps_bool_function(self):
        from gradata.patterns.pipeline import gate, GateResult
        @gate
...
class TestReflection:
    ...
    def test_default_evaluator_scores_non_empty_string(self):
        from gradata.patterns.reflection import default_evaluator, Criterion, CriterionScore
...
class TestGuardrails:
    ...
    def test_pii_detector_catches_email(self):
        from gradata.patterns.guardrails import pii_detector, GuardCheck
    ...
    def test_pii_detector_clean_text_passes(self):
        from gradata.patterns.guardrails import pii_detector
        check = pii_detector.check("The weather is nice today.")
        assert check.result == "pass"

    def test_injection_detector_catches_injection(self):
        from gradata.patterns.guardrails import injection_detector
    ...
    def test_banned_phrases_catches_sycophantic(self):
        from gradata.patterns.guardrails import banned_phrases
    ...
    def test_input_guard_blocks_pii(self):
        from gradata.patterns.guardrails import InputGuard, pii_detector
    ...
    def test_output_guard_passes_clean_output(self):
        from gradata.patterns.guardrails import OutputGuard, banned_phrases
    ...
    def test_guarded_passes_clean_input(self):
        from gradata.patterns.guardrails import guarded, InputGuard, OutputGuard, pii_detector
    ...
    def test_guarded_blocks_pii_input(self):
        from gradata.patterns.guardrails import guarded, InputGuard, pii_detector
    ...
    def test_guarded_result_has_all_passed_field(self):
        from gradata.patterns.guardrails import guarded, GuardedResult
    ...
    def test_destructive_action_guard_catches_drop_table(self):
        from gradata.patterns.guardrails import destructive_action
...
class TestManifestGuardrails:
    ...
    def test_check_write_path_allowed(self):
        from gradata.patterns.guardrails import check_write_path
    ...
    def test_check_exec_command_allowed(self):
        from gradata.patterns.guardrails import check_exec_command
    ...
    def test_scan_for_secrets_clean(self):
        from gradata.patterns.guardrails import scan_for_secrets
    ...
    def test_validate_spawn_no_budget(self):
        from gradata.patterns.guardrails import validate_agent_spawn
...
class TestMemoryScope:
    ...
    def test_prospect_file_is_local(self):
        from gradata.patterns.memory import classify_memory_scope
    ...
    def test_scope_filter_valid(self):
        from gradata.patterns.memory import get_memory_scope_filter
...
class TestMemory:
    ...
    def test_in_memory_store_store_and_retrieve(self):
        import uuid
        from gradata.patterns.memory import InMemoryStore, Memory
        store = InMemoryStore()
    ...
    def test_in_memory_store_missing_returns_empty(self):
        from gradata.patterns.memory import InMemoryStore
        store = InMemoryStore()
    ...
    def test_semantic_memory_conflict_resolve_picks_newer(self):
        from gradata.patterns.memory import SemanticMemory, Memory
        sm = SemanticMemory()
    ...
    def test_procedural_memory_reinforce_increments_count(self):
        from gradata.patterns.memory import ProceduralMemory
        pm = ProceduralMemory()
...
class TestEvaluator:
    ...
    def test_quality_dimensions_nonempty(self):
        from gradata.patterns.evaluator import QUALITY_DIMENSIONS
    ...
    def test_quality_dimensions_are_eval_dimension_objects(self):
        from gradata.patterns.evaluator import QUALITY_DIMENSIONS, EvalDimension
        for d in QUALITY_DIMENSIONS:
            assert isinstance(d, EvalDimension)

    def test_evaluate_returns_eval_result(self):
        from gradata.patterns.evaluator import evaluate, QUALITY_DIMENSIONS, default_evaluator, EvalResult
        result = evaluate(
    ...
    def test_evaluate_empty_dimensions_raises(self):
        from gradata.patterns.evaluator import evaluate, default_evaluator
        with pytest.raises(ValueError):
            evaluate("text", dimensions=[], evaluator=default_evaluator)

    def test_evaluate_has_scores_and_feedback(self):
        from gradata.patterns.evaluator import evaluate, QUALITY_DIMENSIONS, default_evaluator
...
class TestParallel:
    ...
    def test_parallel_batch_runs_all_tasks(self):
        from gradata.patterns.parallel import ParallelBatch, ParallelTask
        batch = ParallelBatch(
    ...
    def test_parallel_batch_empty_returns_empty_results(self):
        from gradata.patterns.parallel import ParallelBatch, ParallelResult
        batch = ParallelBatch()
        result = batch.run()
    ...
    def test_merge_results_combine_strategy(self):
        from gradata.patterns.parallel import merge_results, TaskResult
        results = [
    ...
    def test_merge_results_empty_list_returns_empty(self):
        from gradata.patterns.parallel import merge_results
...
class TestHumanLoop:
    ...
    def test_assess_risk_returns_risk_assessment(self):
        from gradata.patterns.human_loop import assess_risk, RiskAssessment
    ...
    def test_assess_risk_low_for_read_action(self):
        from gradata.patterns.human_loop import assess_risk
    ...
    def test_gate_low_risk_auto_approved_returns_none(self):
        from gradata.patterns.human_loop import gate, assess_risk
        risk = assess_risk("read the file")
        result = gate("read the file", risk=risk, auto_approve_low=True)
        assert result is None

    def test_gate_high_risk_returns_approval_request(self):
        from gradata.patterns.human_loop import gate, assess_risk, ApprovalRequest
        risk = assess_risk("delete all records")
    ...
    def test_preview_action_returns_string(self):
        from gradata.patterns.human_loop import preview_action
...
class TestSubAgents:
    ...
    def test_delegation_has_required_fields(self):
        from gradata.patterns.sub_agents import Delegation
        d = Delegation(
            agent="researcher",
    ...
    def test_orchestrate_dispatches_to_handlers(self):
        from gradata.patterns.sub_agents import orchestrate, Delegation, OrchestratedResult
    ...
    def test_orchestrate_unknown_agent_records_failure(self):
        from gradata.patterns.sub_agents import orchestrate, Delegation
    ...
    def test_orchestrate_empty_delegations(self):
        from gradata.patterns.sub_agents import orchestrate, OrchestratedResult
...
class TestTools:
    ...
    def test_tool_registry_register_and_get(self):
        from gradata.patterns.tools import ToolRegistry, ToolSpec
        reg = ToolRegistry()
...
class TestRag:
    ...
    def _make_chunks(self, texts: list[str], scores: list[float] | None = None) -> list:
        from gradata.patterns.rag import Chunk
        if scores is None:
    ...
    def test_rrf_merge_combines_chunk_lists(self):
        from gradata.patterns.rag import rrf_merge
    ...
    def test_apply_graduation_scoring_boosts_rules(self):
        from gradata.patterns.rag import apply_graduation_scoring, Chunk
        chunks = [
    ...
    def test_apply_graduation_scoring_empty_input(self):
        from gradata.patterns.rag import apply_graduation_scoring
        assert apply_graduation_scoring([]) == []

    def test_order_by_relevance_position_sorts_highest_first(self):
        from gradata.patterns.rag import order_by_relevance_position
    ...
    def test_cascade_retrieve_returns_retrieval_result(self):
        from gradata.patterns.rag import cascade_retrieve, RetrievalResult, Chunk
...
class TestScope:
    ...
    def test_audience_tier_values_exist(self):
        from gradata.patterns.scope import AudienceTier
    ...
    def test_classify_scope_returns_tuple(self):
        from gradata.patterns.scope import classify_scope
    ...
    def test_classify_scope_detects_audience_vp(self):
        from gradata.patterns.scope import classify_scope, AudienceTier
    ...
    def test_register_task_type_adds_to_registry(self):
        from gradata.patterns.scope import register_task_type, classify_scope
        register_task_type(
...
class TestMCP:
    ...
    def test_create_brain_mcp_tools_returns_list(self):
        from gradata.patterns.mcp import create_brain_mcp_tools
        tools = create_brain_mcp_tools()
        assert isinstance(tools, list)
        assert len(tools) >= 3

    def test_create_brain_mcp_tools_have_names(self):
        from gradata.patterns.mcp import create_brain_mcp_tools, MCPToolSchema
        tools = create_brain_mcp_tools()
        for t in tools:
