...
class TestStatsBetaPosterior:
    ...
    def test_naive_bayes_score(self):
        from aios_brain._stats import naive_bayes_score
        base_rates = {
            "_overall": 0.015,
    ...
    def test_bayesian_confidence_update_success(self):
        from aios_brain._stats import bayesian_confidence_update
...
class TestEventsModule:
    ...
    def test_emit_gate_result(self, tmp_path):
        ...
        from aios_brain._events import emit_gate_result
    ...
    def test_emit_gate_override(self, tmp_path):
        brain = _init_brain(tmp_path)
        from aios_brain._events import emit_gate_override
    ...
    def test_supersede(self, tmp_path):
        brain = _init_brain(tmp_path)
        from aios_brain._events import query, supersede
    ...
    def test_supersede_nonexistent(self, tmp_path):
        brain = _init_brain(tmp_path)
        from aios_brain._events import supersede
    ...
    def test_correction_rate(self, tmp_path):
        ...
        from aios_brain._events import correction_rate
    ...
    def test_find_contradictions_no_conflicts(self, tmp_path):
        ...
        from aios_brain._events import find_contradictions
    ...
    def test_audit_trend(self, tmp_path):
        ...
        from aios_brain._events import audit_trend
    ...
    def test_detect_session_from_loop_state(self, tmp_path):
        ...
        from aios_brain._events import _detect_session
...
"""
    ...
    def test_parse_lessons_counts(self):
        from aios_brain._self_improvement import parse_lessons
    ...
    def test_update_confidence_no_corrections(self):
        from aios_brain._self_improvement import parse_lessons, update_confidence
        lessons = parse_lessons("[2026-01-01] [INSTINCT:0.50] DRAFTING: Hyperlink URLs.\n")
    ...
    def test_format_lessons_roundtrip(self):
        from aios_brain._self_improvement import parse_lessons, format_lessons
        original = (
    ...
    def test_graduate_splits_correctly(self):
        from aios_brain._self_improvement import parse_lessons, graduate, LessonState
        text = (
    ...
    def test_compute_learning_velocity_empty(self):
        from aios_brain._self_improvement import compute_learning_velocity
    ...
    def test_compute_learning_velocity_full(self):
        from aios_brain._self_improvement import parse_lessons, compute_learning_velocity
        text = (
            "[2026-01-01] [INSTINCT:0.40] TONE: Be direct.\n\n"
            "[2026-01-02] [PATTERN:0.70] ACCURACY: Check facts.\n\n"
...
class TestQueryModule:
    ...
    def test_fts_index_and_search(self, tmp_path):
        brain = _init_brain(tmp_path)
        from aios_brain._query import fts_index, fts_search
    ...
    def test_fts_index_batch(self, tmp_path):
        brain = _init_brain(tmp_path)
        from aios_brain._query import fts_index_batch, fts_search
        docs = [
    ...
    def test_fts_search_no_results(self, tmp_path):
        brain = _init_brain(tmp_path)
        from aios_brain._query import fts_search
    ...
    def test_detect_query_mode_keyword_quoted(self):
        from aios_brain._query import detect_query_mode
    ...
    def test_reciprocal_rank_fusion_merges(self):
        from aios_brain._query import reciprocal_rank_fusion
        list_a = [
    ...
    def test_compute_recency_weight_fresh(self):
        from aios_brain._query import compute_recency_weight
    ...
    def test_compute_recency_weight_old(self):
        from aios_brain._query import compute_recency_weight, RECENCY_FLOOR
    ...
    def test_classify_confidence(self):
        from aios_brain._query import classify_confidence, CONFIDENCE_HIGH, CONFIDENCE_MED, CONFIDENCE_LOW
    ...
    def test_infer_memory_type_strategic(self):
        from aios_brain._query import infer_memory_type
    ...
    def test_infer_memory_type_fallback_to_map(self):
        from aios_brain._query import infer_memory_type, MEMORY_TYPE_MAP
    ...
    def test_brain_search_keyword_mode(self, tmp_path):
        ...
        from aios_brain._query import fts_rebuild, brain_search
        fts_rebuild()
...
class TestBrainClass:
    ...
    def test_emit_direct_fallback(self, tmp_path):
        """_emit_direct writes to SQLite without the events module."""
        brain = _init_brain(tmp_path)
        result = brain._emit_direct(
            "TEST_DIRECT", "pytest",
            data={"key": "value"},
            tags=["test:tag"],
            session=1,
        )
        assert result["type"] == "TEST_DIRECT"
        assert result["source"] == "pytest"
        assert "ts" in result

        conn = sqlite3.connect(str(brain.db_path))
        row = conn.execute(
            "SELECT source FROM events WHERE type = 'TEST_DIRECT'"
        ).fetchone()
        conn.close()
        assert row is not None
        assert row[0] == "pytest"

    def test_emit_direct_none_data(self, tmp_path):
        """_emit_direct handles None data and None tags without crashing."""
        brain = _init_brain(tmp_path)
        result = brain._emit_direct("NULL_TEST", "pytest", None, None, None)
        assert result["type"] == "NULL_TEST"
    ...
    def test_manifest_fallback_on_import_error(self, tmp_path):
        ...
        def patched_import(name, *args, **kwargs):
            if name == "aios_brain._brain_manifest":
...
class TestExportBrainModule:
    ...
    def test_sanitize_email(self):
        from aios_brain._export_brain import sanitize_content
    ...
    def test_sanitize_filename(self):
        from aios_brain._export_brain import sanitize_filename
        name_map = {"John Smith": "[PROSPECT_1]"}
    ...
    def test_read_version_missing_file(self, tmp_path):
        ...
        try:
            from aios_brain._export_brain import read_version
    ...
    def test_read_session_count_missing_file(self, tmp_path):
        ...
        _p.VERSION_FILE = tmp_path / "nonexistent_VERSION.md"
        try:
            from aios_brain._export_brain import read_session_count
            assert read_session_count() == 0
        finally:
            _p.VERSION_FILE = orig

    def test_count_lessons_missing_file(self, tmp_path):
        from aios_brain._export_brain import count_lessons
    ...
    def test_collect_brain_files_no_prospects(self, tmp_path):
        brain = _init_brain(tmp_path)
        from aios_brain._export_brain import collect_brain_files
...
class TestOnboard:
    ...
    def test_build_manifest_local_embedding(self):
        from aios_brain.onboard import _build_manifest
    ...
    def test_create_db_creates_tables(self, tmp_path):
        from aios_brain.onboard import _create_db
        db = tmp_path / "test.db"
        _create_db(db)
    ...
    def test_create_company_md(self, tmp_path):
        from aios_brain.onboard import _create_company_md
        brain_dir = tmp_path / "brain"
        brain_dir.mkdir()
