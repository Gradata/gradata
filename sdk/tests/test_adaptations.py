...
from gradata.patterns.q_learning_router import (
    QLearningRouter,
    RouteDecision,
    RouterConfig,
)
...
from gradata.enhancements.install_manifest import (
    InstallManifest,
    InstallState,
    Module,
    ModuleCost,
...
class TestMemoryTaxonomy:
    ...
    def test_atomic_fact(self):
        f = AtomicFact(
            facts=["Oliver prefers casual tone", "No em dashes"],
    ...
    def test_brain_profile_merge(self):
        ...
        coding = [f for f in bp1.strengths if f.value == "coding"][0]
    ...
    def test_classify_memory_type(self):
        ...
        assert classify_memory_type("Oliver corrected the email") == MemoryType.CORRECTION_NARRATIVE
...
class TestManifestCapabilities:
    ...
    def test_source_attribution(self):
        caps = _sdk_capabilities()
        modules = caps["modules"]
        assert modules["context_brackets"]["source"] == "paul"
        ...
        assert modules["carl_priority_tiers"]["source"] == "paul+gradata"
...
from gradata.enhancements.eval_benchmark import (
    BenchmarkCase,
    LearningBenchmark,
...
from gradata.patterns.loop_detection import (
    LoopAction,
    LoopDetector,
    LoopDetectorConfig,
)
...
from gradata.patterns.middleware import (
    Middleware,
    MiddlewareChain,
    MiddlewareContext,
    MiddlewareError,
...
class TestManifestNewModules:
    def test_all_new_modules_in_capabilities(self):
        ...
        expected = [
            ...
            "auto_correct_hook", "brain_briefing", "anti_patterns",
            "rule_ab_testing",
        ]
        for name in expected:
...
from gradata.enhancements.brain_briefing import (
    BrainBriefing,
    BriefingRule,
    generate_briefing,
    export_briefing,
...
class TestBrainBriefing:
    ...
    def test_brain_export_briefing(self):
        from gradata.brain import Brain
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
            brain = Brain.init(tmpdir, domain="Test")
            try:
                brain.correct("formal text", "casual text")
            except Exception:
                pass
            result = brain.export_briefing(tmpdir, formats=["generic"])
            assert isinstance(result, dict)
