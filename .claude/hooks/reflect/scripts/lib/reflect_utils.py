...
def get_project_folder_name(project_dir: Optional[str] = None) -> str:
    ...
    C:\\Users\\olive\\Sprites Work → C--Users-olive-Sprites-Work
    """
    project_path = Path(project_dir).resolve() if project_dir else Path.cwd().resolve()
    folder_name = str(project_path)
    # Replace all path separators, colons, and spaces with dashes (matches Claude Code encoding)
...
# NOTE: Adapted from claude-reflect — coding-specific patterns replaced with universal signals
GUARDRAIL_PATTERNS = [
    (r"don't (?:add|include|create|send|change|do) .{1,40} unless", "dont-unless-asked", 0.90, 120),
    (r"only (?:change|modify|edit|touch|do|handle) what I (?:asked|requested|said|told)", "only-what-asked", 0.90, 120),
    (r"stop (?:changing|modifying|doing|adding|removing) (?:unrelated|other|extra|things I didn't)", "stop-unrelated", 0.90, 120),
    (r"don't (?:over-complicate|add extra|be too|make unnecessary|go beyond)", "dont-overcomplicate", 0.85, 90),
    (r"don't (?:change|redo|undo|rework) (?:unless|without)", "dont-change-unless", 0.85, 90),
    (r"leave .{1,30} (?:alone|unchanged|as is)", "leave-alone", 0.85, 90),
    (r"don't (?:assume|guess|make up|fabricate|hallucinate)", "dont-assume", 0.90, 120),
    (r"(?:minimal|minimum|only necessary|just what I asked)", "minimal-changes", 0.80, 90),
    (r"I already (?:told|said|explained|mentioned)", "already-told", 0.85, 120),
    (r"that's not what I (?:meant|asked|said|wanted)", "not-what-i-meant", 0.85, 90),
]
