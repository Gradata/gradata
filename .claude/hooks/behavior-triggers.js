...
if (toolName === 'Bash') {
  ...
  if ((cmd.includes('pytest') || cmd.includes('test')) &&
    ...
    if (shouldFire('test-failure')) {
      triggers.push('TEST FAILURE detected — load skills/systematic-debugging/SKILL.md for root cause analysis. If multiple modules failing, use skills/focused-fix/SKILL.md instead.');
    }
  }
  ...
  if (cmd.includes('git commit')) {
    if (shouldFire('post-commit')) {
      triggers.push('COMMIT made — if ready to merge, use skills/pr-review-expert/SKILL.md for blast radius check. For release prep, use skills/release-manager/SKILL.md.');
    }
  }
  ...
  if (cmd.includes('git push')) {
    if (shouldFire('post-push')) {
      triggers.push('PUSH detected — consider creating a PR with blast radius analysis (skills/pr-review-expert/SKILL.md).');
    }
  }
  ...
  if (cmd.includes('gh pr create') || cmd.includes('gh pr ')) {
    if (shouldFire('pr-created')) {
      triggers.push('PR ACTIVITY — load skills/adversarial-review/SKILL.md for pre-merge debate. Also run skills/pr-review-expert/SKILL.md for structured review.');
    }
  }
}
...
if ((toolName === 'Write' || toolName === 'Edit') && filePath.includes('brain/prospects/')) {
  if (shouldFire('prospect-update')) {
    triggers.push('PROSPECT NOTE updated — verify contact info, next_touch date, and tags are accurate. Load skills/verification-before-completion/SKILL.md.');
  }
}
...
if ((toolName === 'Write' || toolName === 'Edit') && filePath.includes('.claude/hooks/')) {
  if (shouldFire('hook-security')) {
    triggers.push('HOOK MODIFIED — run skills/skill-security-auditor/SKILL.md to verify no config tampering or injection risks.');
  }
}
...
if ((toolName === 'Write' || toolName === 'Edit') &&
  ...
  if (shouldFire('carl-modified')) {
    triggers.push('SYSTEM RULES MODIFIED — run skills/rule-validator/SKILL.md to check for conflicts. Also verify CLAUDE.md line count stays under 150.');
  }
}
...
if ((toolName === 'Write' || toolName === 'Edit') &&
  ...
  if (shouldFire('email-draft')) {
    triggers.push('PROSPECT-FACING CONTENT written — load skills/adversarial-review/SKILL.md for CRITIC/DEFENDER debate before sending.');
  }
}
...
if ((toolName === 'Write' || toolName === 'Edit') &&
  ...
  if (shouldFire('dep-change')) {
    triggers.push('DEPENDENCY FILE changed — run skills/dependency-auditor/SKILL.md to check for license conflicts and vulnerabilities.');
  }
}
...
if ((toolName === 'Write' || toolName === 'Edit') && filePath.includes('skills/') && filePath.includes('SKILL.md')) {
  if (shouldFire('skill-written')) {
    triggers.push('SKILL MODIFIED — run skills/skill-tester/SKILL.md to validate structure, and update skill-router.js if new intent patterns needed.');
  }
}
