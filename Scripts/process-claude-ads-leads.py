import json, csv, os, re, sys, urllib.request
sys.stdout.reconfigure(errors='replace')

# Download all profiles
dataset_id = 'IQrmvK1mWBFeN1atg'
all_items = []
offset = 0
limit = 100
while True:
    url = f'https://api.apify.com/v2/datasets/{dataset_id}/items?offset={offset}&limit={limit}&format=json'
...
print(f'Downloaded {len(all_items)} profiles')

# Save raw
wip = 'C:/Users/olive/OneDrive/Desktop/Sprites Work/Leads/wip/claude-ads-post-scrape'
with open(f'{wip}/claude-ads-profiles-raw.json', 'w', encoding='utf-8') as f:
    json.dump(all_items, f, ensure_ascii=False)

# Clean output dir BEFORE dedup scan (so we don't dedup against our own previous output)
out_dir = 'C:/Users/olive/OneDrive/Desktop/Sprites Work/Leads/active/claude-ads-post-aditi'
if os.path.exists(out_dir):
    for old_fn in os.listdir(out_dir):
        if old_fn.endswith('.csv'):
            os.remove(os.path.join(out_dir, old_fn))

# Load ALL existing lead URLs for cross-post dedup
existing_urls = set()
active_base = 'C:/Users/olive/OneDrive/Desktop/Sprites Work/Leads/active'
for root, dirs, files in os.walk(active_base):
    for fn in files:
        if fn.endswith('.csv'):
            try:
                with open(os.path.join(root, fn), 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        u = (row.get('linkedin_url') or '').lower().rstrip('/')
...
for p in all_items:
    ...
    results.append({
        ...
        'icp_tier': tier,
        'geo_bucket': geo,
        'source': 'claude-ads-post-aditi'
    })

print(f'Total profiles: {stats["total"]}')
print(f'Deduped (cross-post): {stats["dedup"]}')
...
# Split
na = [r for r in results if r['geo_bucket'] == 'na' and r['icp_tier'] != 'WHITELABEL']
intl = [r for r in results if r['geo_bucket'] == 'intl' and r['icp_tier'] != 'WHITELABEL']
wl = [r for r in results if r['icp_tier'] == 'WHITELABEL']
unknown_geo = [r for r in results if r['geo_bucket'] in ('unknown-geo','no-geo') and r['icp_tier'] != 'WHITELABEL']
...
written_total = len(na_t1)+len(na_t2)+len(na_t3)+len(intl_t1)+len(intl_t2)+len(intl_t3)+len(wl)+len(unknown_geo)
print(f'NA: T1={len(na_t1)}, T2={len(na_t2)}, T3={len(na_t3)}')
print(f'INTL: T1={len(intl_t1)}, T2={len(intl_t2)}, T3={len(intl_t3)}')
print(f'WL: {len(wl)}, Unknown-geo: {len(unknown_geo)}')
print(f'VERIFY: {written_total} written == {len(results)} survivors: {"MATCH" if written_total==len(results) else "MISMATCH"}')
...
for name, rows in [
    ...
    (f'04-UNKNOWN-GEO-{len(unknown_geo)}-leads', unknown_geo),
]:
    if rows:
        write_csv(os.path.join(out_dir, name + '.csv'), rows)
...
for fn in sorted(os.listdir(out_dir)):
    ...
    if fn.endswith('.csv'):
        with open(os.path.join(out_dir, fn), 'r', encoding='utf-8') as f:
            total_in_files += sum(1 for _ in f) - 1
print(f'\nTotal leads in files: {total_in_files}')
